...
def get_samples(filenames, remove_prefixes=None):
    for path in filenames:
        ...
        if len(reads) == 1:
            yield {
                'name': sample_name,
                'reads1': Path(reads[0]).resolve()
            }
        else:
            yield {
                'name': sample_name,
                'reads1': Path(reads[0]).resolve(),
                'reads2': Path(reads[1]).resolve()
            }


def add_samples(database, files, fofn, remove_common_path, create,
                remove_prefix=[], force=False, *args, **kwargs):
...
def main():
    ...
    create_parser.add_argument(
        '-r', '--remove-prefix', action="append", default=[], required=False,
        help="Remove the given prefix from each path. This option can be used "
             "multiple times to remove multiple prefixes."
    )
