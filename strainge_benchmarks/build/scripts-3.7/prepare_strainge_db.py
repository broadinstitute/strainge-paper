#!/cil/shed/apps/internal/conda/envs/strainge/bin/python
"""
...
The output is a TSV file with all found reference genomes, a generated ID and
other metadata. This file is then used throughout the Snakemake workflow.
"""
...
def split_chrom_plasmids(path):
    ...
    chromosomes = []
    plasmids = []
    with func(path, "rt") as f:
        for entry in SeqIO.parse(f, "fasta"):
            logger.debug('%s %s (length: %d)', entry.id, entry.description,
                         len(entry.seq))

            if "plasmid" in entry.description or len(entry.seq) < 500e3:
    ...
    with gzip.open(chrom_path, "wt") as f:
        SeqIO.write(chromosomes, f, "fasta")
